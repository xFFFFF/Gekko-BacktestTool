...
if ($oimportcc) {
  print "[".strftime ("%Y-%m-%d %H:%M:%S", localtime)."] Import via CryptoCompare API.\n";
  my $startapp = time;
  ...
  foreach (@pairs) {
    my @sets = split /:/, $_;
    my $date = time;
    print "[".strftime ("%Y-%m-%d %H:%M:%S", localtime)."] Import $sets[0]:$sets[1] is stared at ".strftime ("%Y-%m-%d %H:%M:%S", localtime)."\n";
    my $startpair = time;
    my $dbh = DBI->connect("dbi:SQLite:dbname=history/$sets[0]_0.1.db", "", "",{ RaiseError => 1, PrintError => 1, AutoCommit => 0 },);
    my $query = qq(CREATE TABLE IF NOT EXISTS candles_$sets[1]_$sets[2] (id INTEGER PRIMARY KEY AUTOINCREMENT, start INTEGER UNIQUE, open REAL NOT NULL, high REAL NOT NULL, low REAL NOT NULL, close REAL NOT NULL, vwp REAL NOT NULL, volume REAL NOT NULL, trades INTEGER NOT NULL));
    my $sth = $dbh->prepare($query);
    my $rv = $sth->execute();
    for my $i (1 .. 6) {
      my $response = $req->get("https://min-api.cryptocompare.com/data/histominute?fsym=$sets[2]&tsym=$sets[1]&limit=2000&e=$sets[0]&aggregate=1&toTs=$date&extraParams='Gekko BacktestTool'");
      if ($response->is_success) {
        ...
        foreach (@{ $response->{'Data'} }) {
          ...
          my $open = $_->{'open'};
          my $close = $_->{'close'};
          my $vwp = $high + $low + $open + $close / 4;
          my $volume = $_->{'volumeto'};
          my $trades = 1;
          my $query = qq(INSERT OR IGNORE INTO candles_$sets[1]_$sets[2] values (null, $start, $open, $high, $low, $close, $vwp, $volume, $trades));
          my $sth = $dbh->prepare($query);
          my $rv = $sth->execute();
        }
        $date = $response->{'Data'}->[0]->{'time'};
        print "[".strftime ("%Y-%m-%d %H:%M:%S", localtime)."] Importing candles for $sets[1]:$sets[2] \@ $sets[0] from ".strftime ("%Y-%m-%d %H:%M:%S", localtime($response->{'TimeFrom'}))." to ".strftime ("%Y-%m-%d %H:%M:%S", localtime($response->{'TimeTo'}))."\nhistominute?fsym=$sets[2]&tsym=$sets[1]&limit=2000&e=$sets[0]&aggregate=1&toTs=$date\n";
        print $response->{"Response"};
        if ($response->{"Response"} eq "Error") {
          print "[".strftime ("%Y-%m-%d %H:%M:%S", localtime)."] Import failed. CryptoCompare error: $response->{'Message'}\n";
        }
      }
    }
    $dbh->commit() or die $dbh->errstr;
    print "[".strftime ("%Y-%m-%d %H:%M:%S", localtime)."] Import $sets[1]:$sets[2] is done. Elapsed ".elapsed(time - $startpair)."\n" if $stfu eq 'no';;
  }
  print "[".strftime ("%Y-%m-%d %H:%M:%S", localtime)."] All is done. Elapsed time: ".elapsed(time - $startapp)."\n" if $stfu eq 'no';
  exit
}
